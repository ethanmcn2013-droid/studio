...
- Signal Timeline — status pill, public workspace BigStats (both header
...
- Signal Timeline — Tier 3 calm-attention layer lands (R·20). A pure
...
- Signal Timeline — per-row attention indicator lands (R·21). ItemRow
...
- Signal Timeline — drift now surfaces at edit time too (R·22). The
...
- Signal Timeline — DB rename closes the thread end-to-end (R·23). The
...
- Signal — three briefing engine sites where "blocked"
...
the signal layer, not a fifth manual lane. Adding a 5th lane would
...
**Refusal — Tasks per-row pill on the board (parity with Timeline R·21,
...
Tasks board for parity with Timeline ItemRow. We refuse, for three
reasons:
...
1. **My Week is the briefing.** Tasks already ships the calm-attention
   ...
   *as* the answer to "how does an owner see what's drifting?" Adding a
   ...
   Timeline needed per-row because Timeline has no My Week equivalent —
   the public timeline *is* the owner's main surface.
2. **The board is for state movement, not signal.** A Tasks board
   ...
   lanes. Timeline's ItemRow is a read surface; the Tasks board is an
   edit surface — the signal layer belongs on the read surface.
3. **The thresholds disagree on purpose.** Timeline idle = 14 days
...
exactly one canonical place per product. Timeline → ItemRow + workspace
BigStat. Tasks → My Week "Needs attention" bucket. Notes → silence.
Signal → briefing phrasing. No parity for parity's sake.
