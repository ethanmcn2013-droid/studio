...
**Drift cleared 2026-05-16 (same day).** The pricing/venue surfaces have been brought onto this decision: `signalstudio.ie/pricing` now carries a Venue Edition patronage section + Pro €120/yr annual prepay; `signalstudio.ie/venues` rebuilt free→paid (every "free / with our compliments / the gift / no card" string removed); `docs/VENUE_EDITIONS_PLAN.md` carries an AMENDED-2026-05-16 banner pointing here as canonical. Architecture also shipped: the `sponsors` ledger gained paid/term/founding-lock/allotment fields (Drizzle migration `0003_venue_paid_tier`), HQ Traction now reports cash-collected from paid venue records (not couple entitlements) against the €250k goal, `scripts/mark-venue-paid.ts` is the operator path (deliberately not a self-serve checkout — a public "buy €4,000" button is the SaaS register BRAND.md §3 forbids), and `docs/shipped-state.md` (the factory's Reality-Anchor source) was written. **Live as of 2026-05-16:** the paid model is on prod — `signalstudio.ie/venues` and `/pricing` verified serving the patronage copy + €1,500–€4,000 + €120/yr (Studio branch `feat/hq-mission-control-2026-05-16` @ `672d08e`, prod-aliased). The venue-ledger columns are applied + verified in BOTH prod Turso DBs (studio + signal-entitlements). Tasks `/api/checkout` is interval-aware with graceful monthly fallback on branch `feat/workspace-annual-checkout-2026-05-16` (pushed, PR-ready). **Irreducible operator steps remaining (not code — billing-account + merge decisions):** (1) create the €120/yr Stripe Price and set `STRIPE_PRICE_WORKSPACE_ANNUAL` on Tasks Vercel prod; (2) merge the Tasks annual branch to `main` for its prod deploy. Until (1), the annual link degrades to monthly checkout — nothing is broken. Success is judged on five leading indicators (≥10 paid venues by M3, ≥25% close rate, ≥1 referral inbound by M4, ≥40% sponsored-couple activation, zero brand-integrity exceptions), not a vanity €500k.
