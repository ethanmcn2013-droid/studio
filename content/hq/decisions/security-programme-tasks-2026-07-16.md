...
The first remediation tranche now has code and verification evidence: destructive workspace mutations use a server-side owner boundary, pending invite listing is owner-only and omits bearer tokens, production checkout fails closed when Stripe is unavailable, and authenticated calendar responses are private/no-store. `pnpm test`, TypeScript checking and `next build` passed on 2026-07-16.
