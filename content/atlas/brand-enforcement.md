...
- 2026-05-22 (S·65/S·66/S·67 + wordmark/loader fixes batch) — `src/components/brand/wordmark.tsx`, `src/app/brand/page.tsx`, `src/app/globals.css`, and DESIGN.md all carry recent touches across this two-week run (mobile loader X-Frame scope-override `e503c25`, loader iframe phone-frame scaling `2b6fc11`, static top-left wordmark removal on mobile `c05d831`, S·66 iOS Today doc IA spec `d554321` adding `docs/strategy/IOS_TODAY_DOC_IA_2026_05_21.md` + `/api/native/today` route + shape-native test, S·67 `/templates` audience-pill row `e566c07` adding a byte-portable primitive). Re-read all four files against §3 voice, §5 visual register, §6 page conventions, and the refusal list. Catch-net mechanism unchanged: BRAND.md remains the operational rule, the operator remains the runtime, the banned-word list is intact, the indigo accent `#4f46e5` and paper/ink tokens still match globals.css, the locked H1 "Project management for the 80% not in tech." still renders, the wordmark grammar (`signal studio.` with the period) and the five-gesture vocabulary (broadcast/pulse/sweep/tick/caret) are unchanged in `wordmark.tsx`. CHANGELOG-and-cosmetic drift only; the rule-system the entry documents is intact.
