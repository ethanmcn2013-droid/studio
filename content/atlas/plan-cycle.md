...
lens: Processes
owner: Ethan
lastVerified: 2026-05-26T18:00Z
...
- 2026-05-15 (A·5) — Signal CHANGELOG.md grew an A·5 dispatch entry for the code-review remediation. CHANGELOG-only drift; the dispatch is the loop working exactly as documented (step 4: cycle closed → entry in product voice). Re-verified, body unchanged.
...
- 2026-05-26 (S·68 atlas-only) — CHANGELOG.md grew the S·68 dispatch entry (/venues rebuild). Standard A.11 re-flag: CHANGELOG.md is a referenced file. Re-verified: the five-step cycle loop is operating exactly as documented — phase.md still names the active cycle (Suite product-perfection sprint), STATUS blocks still close every turn (the Stop hook enforces this), log.jsonl still records, the dispatch is still where shipped work surfaces. S·68 is itself a textbook cycle close: venue page rebuilt in a session, CHANGELOG entry written in product voice, memory would be updated with anything surprising. BRAND.md §6.6 (operating vocabulary — pass/step/update/fix/re-do/push/look-back/look-ahead/mark/week) was also committed in S·68, adding an operator-facing layer to the plan-cycle vocabulary. The §6.6 vocabulary is additive: it names what was already practised (the X·NN cycle codes, the STATUS block, the phase line) in the 80%-audience register. The five-step loop body and its two invariants (STATUS block non-negotiable; phase.md never edited silently) are unchanged. CHANGELOG-only drift; atlas-only commit. `core.hooksPath` confirmed `.githooks`.
- 2026-05-26 (T·78) — Tasks CHANGELOG.md grew the T·78 dispatch entry (toolbar icon-only + conversation author names from DB JOIN). Standard A.11 re-flag: Tasks CHANGELOG.md is a referenced file. Re-verified: the cycle loop is operating as documented — phase.md still names the active cycle, STATUS blocks still close every turn, log.jsonl still records, the dispatch is where shipped work surfaces. T·78 is a textbook tightens entry: two quality improvements in one commit, CHANGELOG entry in product voice, no shape change to the loop. CHANGELOG-only drift; loop body unchanged. `core.hooksPath` confirmed `.githooks`.
