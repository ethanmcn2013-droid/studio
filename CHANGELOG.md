...
## 2026-07-08 · S·119 · ships · the front door becomes a waitlist

**Signal Studio now has one public conversion path: join the waitlist.** `/waitlist` is a real page with a short form, staged-access copy, source tracking, duplicate-safe submissions, and a `waitlist_entries` ledger behind it. Public CTAs across homepage products, pricing, weddings, venues, templates, proof, press, comparisons, footer, navigation, review, and recovery routes now point there instead of asking people to open products before access is ready.

Signal HQ gets `/hq/waitlist` so the list is visible inside the operating system, and public structured data now describes access as preorder rather than in stock. The remaining product-origin links are deliberately not marketing CTAs: authenticated suite navigation, issued-code redemption, internal data, or example proof links.

## 2026-07-08 · S·118 · ships · Founder Copy Review enters HQ

**Signal HQ now has a copy governance room: every meaningful line can be scanned, reviewed, approved, rejected, deferred, or promoted into guidance without pretending the approval survives a rewrite.** The room binds approval to the exact text hash, so a changed sentence returns to review on the next scan. It opens with 7,370 source-backed copy items, high-risk and legal/pricing filters, weekly review mode, search presets, diff context, founder comments, reusable guidance, and a Hall of Fame for lines worth teaching back to future writing passes.
...
**The homepage product rows stop acting like four mini feature explainers and read as one-sentence work statements: keep the thought, keep ownership clear, show where things stand, show what actually needs you today.** The right rail now cuts the overexplained clauses and gives each product one direct job. The timeline link in the meta row also holds together, so the label no longer breaks across lines.
...
The pricing surface also stops carrying stale commercial truth: the ongoing paid tier is Pro, Event is €89 one-time, and the four product labels say In development instead of overstating availability. The checkout parameter stays `workspace` for the entitlement layer; the public plan name does not have to leak the implementation name.
