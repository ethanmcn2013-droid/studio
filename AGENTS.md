...
- **`~/Projects/personal/BRAND.md`** — full voice/brand handbook. Lives outside this repo because it's the source-of-truth for the whole suite. Read it when in doubt about voice or audience tone. (Codex won't have direct read access to it from outside the repo — when it matters, the relevant excerpt should be reproduced here or in the PR description.)
- **`CHANGELOG.md`** (in this repo) — narrative log of suite-level launches. Add an entry for any user-visible change.
