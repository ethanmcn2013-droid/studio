# Demo Library

Demo assets show real product states and real coordination scenes. Avoid abstract product montages. Show the actual workspace.

## Monthly Cadence

Ship two strong videos per month:

- One product proof: a real flow across Notes, Tasks, Timeline, or Signal.
- One founder or brand piece: refusal list, pricing shape, venue patronage, or why the 80% needs a different register.

## Priority Demos

1. **One wedding Tuesday.** Venue call -> tasks -> public plan -> morning briefing. Public route: `/venues/demo`.
2. **A briefing, not a dashboard.** Show the Daily Signal as a short read, not a metric screen.
3. **The plan your client can read.** Signal Timeline for a couple, client, or supplier with no account.
4. **Not everything is ready for the room.** Signal Notes as private capture before work becomes work.
5. **What Signal Studio refuses.** Founder voice over `/principles` and product refusals.

## 60-Second Wedding Demo Script

**0:00 - The venue call.** A planner writes the live note: venue date, florist, marquee decision, save-the-dates, catering headcount.

**0:18 - The work becomes visible.** Three notes become tasks in the wedding workspace. No setup, no project-manager vocabulary.

**0:38 - The couple gets one link.** The public roadmap shows what changed and what happens next. The couple reads it on a phone.

**0:52 - The next morning.** The briefing names two things: the florist has gone quiet, and save-the-dates are late.

**Closer.** "The venue paid for a clearer planning year. The couple never saw a price."

## 20-Second Product Clips

- **Signal Notes:** "Write it here first." Capture field, stream, approved extract.
- **Signal Tasks:** Wedding template, board/list/timeline/calendar over the same work.
- **Signal Timeline:** One public plan, no login, no private note exposure.
- **Signal:** Three items per block, hard cap, one morning read.

## Production Rule

AI can help draft scripts, shot lists, captions, and edit notes. It is not the public claim. The finished video sounds like the founder and shows shipped product.
