...
export function renderContents(p: DotPose, options: RenderOptions = {}) {
  ...
  const bg =
    options.stage === "transparent"
      ? ""
      : `<rect x="-95" y="-95" width="190" height="190" fill="${dark ? "#1b1b28" : "#f9f9fb"}"/>`;
  const ground = options.grounded
    ? `<ellipse data-part="shadow" cx="${n(p.x)}" cy="52" rx="${n((20 - clamp(-p.y / 30) * 9) * scale)}" ry="1.8" fill="${dark ? "#000000" : "#353146"}" opacity="${n(0.085 * (1 - clamp(-p.y / 45)))}"/>`
    : "";
  const eyes =
    options.face === false
      ? ""
  ...
  const bead =
    effects && p.bead > 0.001
      ? `<circle cx="40" cy="-37" r="${n(6 * p.bead)}" fill="#62c8c7" stroke="${dark ? "#1b1b28" : "#f9f9fb"}" stroke-width="2"/>`
      : "";
