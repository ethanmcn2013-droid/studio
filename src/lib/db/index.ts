 ...
 * STUDIO_DATABASE_URL is unset (the Preview environment, which
...
function createDb() {
  ...
  if (!url) {
    throw new Error(
      "STUDIO_DATABASE_URL is not set. Create a Turso database (turso db create ethanmcnamara-studio) and add the URL to .env.local. See docs/CYCLE_8_1_ENTITLEMENTS_HANDOFF.md.",
    );
  }
