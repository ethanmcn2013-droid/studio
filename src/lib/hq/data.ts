...
export const seedHqData: HqData = {
  ...
  products: [
    ...
    },
    {
      ...
      integrationScore: 48,
      launchReadiness: 54,
      majorFeatures: ["cinematic homepage demo (cycle 11.4)", "Phase A · Settings + opt-out (2026-05-13) — Clerk auth wired into root layout, /app/* middleware-protected, Turso/libSQL via Drizzle with user_preferences table (userId · email · cadence · unsubscribeToken · lastSentAt), /app/settings/notifications page (three radios: daily / weekly default / off, auto-save), public /u/[token] landing + RFC 8058 /api/unsubscribe/[token] one-click POST handler + GET redirect, token rotation on every unsubscribe"],
      blockers: ["Claims must stay tied to what the private preview proves — the cycle 11.4 demo dramatizes a 'Why this?' rule-chain expansion and a 'Mark done' acknowledge gesture that the production briefing doesn't yet expose. Marketing-side reconciliation owed.", "Operator setup for Phase A before any /app route boots: turso db create signal-analytics, TURSO_ANALYTICS_DATABASE_URL + TURSO_ANALYTICS_AUTH_TOKEN (both Sensitive) + Clerk publishable+secret keys on Vercel, npm run db:push locally once."],
      notes: "Phase A shipped 2026-05-13 (commit 0251077 on analytics:main). v1 contract locked with owner: default cadence weekly (brand-faithful low-noise), why-this in email is web-only (email stays one short read), single 06:00 UTC send slot. Built kill-switch BEFORE send pipeline by design — RFC 8058 one-click unsubscribe live now even though no email sends yet, so Phase C can plumb it on. Phase B (engine) and Phase C (Resend + cron fanout) sit on top of A without re-shaping it. Cycle 11.4 cinematic demo still live on marketing.",
      nextActions: ["Operator: turso db create signal-analytics + URL/token to Vercel + Clerk keys + npm run db:push (gates everything else).", "Phase B · Engine: Tasks DB read adapter (read-only Turso token to ethanmcnamara-tasks), 4 triggers (stuck-work / due-soon / just-shipped / overload), prose library v1 (3 phrasings each), Suggested Focus ranking, buildBriefing(userId).", "Phase C · Render + deliver: React Email <BriefingEmail/>, dispatchBriefing(to, briefing) Resend wrapper, /api/cron/daily + /api/cron/weekly fanout filtered by user.cadence, List-Unsubscribe + List-Unsubscribe-Post headers.", "Phase D · In-app surface: /app/brief web mirror (same render tree as email, full-fidelity with why-this expansions), /app/preview-email for visual QA.", "Audit demo-vs-reality: ship 'Why this?' as a real product feature, or temper the marketing-side claim."],
    },
    {
      id: "notes",
      name: "Signal Notes",
