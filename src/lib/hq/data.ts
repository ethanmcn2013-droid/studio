...
export const seedHqData: HqData = {
  version: 1,
  updatedAt: "2026-05-15T02:30:00Z",
  focus: {
    stage: "Pre-launch",
    weekOf: "2026-05-11",
  ...
  products: [
    ...
    },
    {
      ...
      notes: "Live end-to-end as of 2026-05-13 (commit 9a36514 on analytics:main): real Tasks DB read (Phase B.2) joining by email, real briefing engine, real React Email render with calm subject + plain-text alt + Reply-To, real Resend dispatch with RFC 8058 List-Unsubscribe headers + per-send token rotation, real Vercel cron at 06:00 UTC daily. Full kill-switch wired through (footer link + Gmail native unsubscribe button + brand-faithful 'You're off.' landing). /app shell shipped with suite-launcher chrome + Send-test-now button on settings. Cycle 11.4 cinematic demo still live on marketing. Cinematic /app/brief polish + activities-table-driven movedToShippedAt + 'Why this?' as real product feature all deferred to later cycles. Suite Hardening Pass 2026-05-12 (commit 90c8774): unsubscribe-token rotation moved into Resend success branch (was rotating BEFORE send — failed sends stranded users with dead unsubscribe URLs from prior emails); cron auth now timing-safe (constant-time comparison); dead GET handler on cron route removed; mockBriefingSource fallback removed so unconfigured envs render empty state instead of leaking wedding demo data to real users; /method copy aligned to four shipped triggers (was claiming 'ten patterns' + 'fifty-five phrasings' — actual is 4 + 12); /signal team-level fiction removed (SPOF / 'team activity' claims that mapped to no shipped trigger).",
      nextActions: ["Owner: click 'Not spam' on tonight's test email in Gmail to rebuild sender reputation; rotate CRON_SECRET (current one in chat history — burned); toggle 'Sensitive' flag in Vercel dashboard for 5 encrypted env vars; delete test row in user_preferences once signed in via Clerk to /app.", "Phase B.3 · activities-table read for accurate movedToShippedAt timestamps (replaces v1 heuristic).", "Phase E · personalisation: Clerk firstName in greeting ('Good morning, Ethan.'); Clerk user lookup per cron iteration.", "Audit demo-vs-reality: 'Why this?' web view exposes per-item reasons[]; cycle 11.4 demo still dramatizes more than the engine produces. Either ship the additional reasoning surfaces or temper the demo.", "Future cycle: middleware → proxy convention migration (Next 16 deprecation warning during build)."],
    },
    {
      id: "notes",
      name: "Signal Notes",
