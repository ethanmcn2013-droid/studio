...
export const seedHqData: HqData = {
  ...
  focus: {
    stage: "Pre-launch",
    weekOf: "2026-05-11",
    theme: "Sprint 1 · make live Notes private by design.",
    focus:
      "Signal HQ is live and Notes is now on the current Next.js / Clerk / Turso main branch. This cycle sharpens the live notebook as a private cognitive layer: quiet empty-state copy, approved-extract language, and raw-note privacy guardrails.",
    priorities: [
      "Ship the private Notes empty-state in the live `/app` notebook.",
      "Make Notes copy and docs explicit that raw notes are private by default.",
      "Keep collaboration growth built on approved outputs, not exposed private context.",
    ],
    risks: [
      "The future Notes -> Tasks edge could leak trust if it sends full note bodies instead of approved extracts.",
      "Sprint 1 still has 9.3-9.5 to land before Sprint 2 (Analytics live briefing) is the right move.",
      "If Notes starts feeling collaborative, it weakens the suite's public/private model.",
    ],
    nextActions: [
      "Define the extract-level permission contract for Notes -> Tasks.",
      "Run a signed-in production smoke test on notes.signalstudio.ie once the deployment completes.",
      "Decide whether Cycle 9.3 should next handle first-sign-in empty state or extract permissions.",
    ],
  },
  products: [
    {
    ...
    },
    {
      ...
      integrationScore: 34,
      launchReadiness: 62,
      majorFeatures: ["Workspace creation", "editor", "public viewer", "shared update page", "milestones", "cinematic homepage demo (cycle 11.3)"],
      ...
      notes: "Cycle 11.3 cinematic demo shipped — Tasks-pattern hero + AudienceToggle (wedding/construction/launch/startup) + 18-scene loop with reader cursors, comment thread, view morph List ↔ Timeline, share-copy toast, RSS subscriber arrival. Live URL still needs visual verification.",
      nextActions: ["Walk the live roadmap.signalstudio.ie demo and confirm cursor + view-morph positioning on mobile widths.", "Reconcile the demo's comment-thread scene against the locked 'no comment threading' refusal — either soften the refusal or temper the demo.", "Use the shared update in the wedding demo.", "Add richer change history."],
    },
    {
      id: "analytics",
      ...
      integrationScore: 48,
      launchReadiness: 52,
      majorFeatures: ["Briefing engine", "daily email path", "Tasks read model", "signal cards", "cinematic homepage demo (cycle 11.4)"],
      blockers: ["Claims must stay tied to what the private preview proves — the cycle 11.4 demo dramatizes a 'Why this?' rule-chain expansion and a 'Mark done' acknowledge gesture that the production briefing doesn't yet expose. Marketing-side reconciliation owed."],
      notes: "Cycle 11.4 cinematic demo shipped — Tasks-pattern hero + AudienceToggle (wedding/construction/launch/startup) + sender chrome + 19-scene loop with reader cursor, 'Why this?' rule chain with character-typed final line, acknowledge gesture, Today/Yesterday view-morph, per-item provenance trail.",
      nextActions: ["Walk the live analytics.signalstudio.ie demo and confirm cursor + Why-this expansion read correctly.", "Audit demo-vs-reality: ship 'Why this?' as a real product feature, or temper the marketing-side claim.", "Name the universal briefing pattern.", "Connect risk signals to HQ data."],
    },
    {
      id: "notes",
      name: "Signal Notes",
      ...
      integrationScore: 38,
      launchReadiness: 64,
      majorFeatures: ["Next.js 16 scaffold", "Turso + Drizzle server persistence", "suite-wide Clerk auth (live)", "auth-gated /app", "sign-in / sign-up routes", "notebook surface with optimistic UI", "private empty-state copy", "private-by-default product treatment", "collapsible search rail with ⌘K", "wordmark gesture", "anti-feature register", "homepage", "wedding planning demo"],
      blockers: ["Approved Notes -> Tasks extraction endpoint and UI wiring is next (Cycle 9.4b, cross-repo into Tasks). The cycle 11.5 marketing demo now dramatizes this gesture — production implementation owed.", "First-sign-in onboarding empty state could be sharper (Cycle 9.3).", "FTS5 server-side search is a later polish — client-side filter is fast for corpora < 1000 notes."],
      notes: "Cycle 11.5 cinematic demo shipped — Tasks-pattern hero + AudienceToggle (wedding/construction/launch/startup) + 17-scene loop with animated tag-chip landing, Stream ↔ Tags view morph, live search substring highlighting, and the locked Notes → Tasks promote gesture (long-press → menu → flying card silhouette → TasksEdge indicator). The one-way discipline is now legible in motion. Codex's privacy reframe (live notebook private by design) landed earlier in the day.",
      nextActions: ["Walk the live notes.signalstudio.ie demo and confirm the promote-flight + TasksEdge positioning.", "Ship the real Notes → Tasks extraction (cycle 9.4b) so the marketing demo and product match.", "Define extract-level permissions for Notes -> Tasks.", "Cycle 9.3: first-sign-in onboarding empty state."],
    },
  ],
  ecosystemFlows: [
    {
