...
export const seedHqData: HqData = {
  ...
  products: [
    ...
    },
    {
      ...
      integrationScore: 52,
      launchReadiness: 68,
      majorFeatures: ["cinematic homepage demo (cycle 11.4)", "Phase A · Settings + opt-out (2026-05-13) — Clerk auth, /app/* middleware, Turso/libSQL user_preferences, /app/settings/notifications (daily/weekly default/off), public /u/[token] landing, RFC 8058 /api/unsubscribe/[token] one-click POST + GET, token rotation on every unsubscribe", "Phase B.1 · Briefing engine + /app/brief on mock data (2026-05-13) — 4 triggers (stuck-work · due-soon · just-shipped · overload), 3 prose phrasings each rotating per (user, day), Suggested Focus ranking, hard 3-cap per bucket, shared <BriefingView/> render tree, BriefingSource interface (mock-source for v1, Tasks DB read swappable as one-line change)", "Phase C · Email + cron + List-Unsubscribe (2026-05-13) — <BriefingEmail/> via @react-email/components matching <BriefingView/> hierarchy, dispatchBriefing() refuses on empty briefing + no-key, rotates unsubscribeToken on every send, RFC 8058 List-Unsubscribe + List-Unsubscribe-Post:One-Click headers, /api/cron/briefings Bearer auth (daily always + weekly on Mondays UTC), vercel.json cron 0 6 * * *, /app/preview-email sandboxed iframe of actual render"],
      blockers: ["Claims must stay tied to what the private preview proves — cycle 11.4 demo dramatizes 'Why this?' rule-chain + 'Mark done' acknowledge gesture; production briefing exposes them on web only (Phase B.1 BriefingView), email skips them by locked v1 contract.", "Operator setup before anything boots/sends: turso db create signal-analytics + TURSO_ANALYTICS_DATABASE_URL + TURSO_ANALYTICS_AUTH_TOKEN (Sensitive) + Clerk publishable+secret keys + RESEND_API_KEY (Sensitive) + RESEND_FROM + CRON_SECRET (Sensitive) on Vercel; npm run db:push locally once. DKIM for signalstudio.ie still pending.", "Phase B.2 still owed — TasksDbSource that swaps mockBriefingSource for a real read of the user's connected Tasks workspace. Engine + render + cron all already work against the BriefingSource interface; B.2 is a one-line source change once the read-only Turso token to ethanmcnamara-tasks is provisioned."],
      notes: "Phases A, B.1, C shipped 2026-05-13 (commits 0251077, 8b83c5a, 2cf082f on analytics:main). v1 contract: weekly-default, why-this web-only, 06:00 UTC single slot, hard 3-cap per bucket. Kill-switch built BEFORE send pipeline (Phase A) — RFC 8058 one-click unsubscribe live before any cron fires. Engine (Phase B.1) is deterministic per (user, day); 4 triggers + 3 prose phrasings each. Email (Phase C) refuses to send on empty briefings (brand promise) and on missing Resend key (graceful dev). Single Vercel cron at 06:00 UTC handles both cadences (weekly fires Mondays only). Cycle 11.4 cinematic demo still live on marketing.",
      nextActions: ["Operator: provision all env vars on Vercel + npm run db:push + verify domain DKIM. Once done, /api/cron/briefings becomes live and a test cron run can confirm end-to-end.", "Phase B.2 · TasksDbSource: read-only Turso token on ethanmcnamara-tasks, libsql client query of tasks/workspaces/workspace_members tables joined by Clerk userId, map to TaskSignal shape, swap mockBriefingSource for tasksDbSource in /app/brief + /api/cron/briefings (one line each).", "Phase D · In-app shell: /app layout with nav (Briefing · Settings · Sign out), 'Send test now' button on /app/settings/notifications, /app/brief polish toward marketing-cinematic feel.", "Audit demo-vs-reality: ship 'Why this?' as a real product feature, or temper the marketing-side claim."],
    },
    {
      id: "notes",
      name: "Signal Notes",
