...
export function PrototypePicker({
  ...
  useLayoutEffect(moveHighlight, [active, names.length]);
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (number >= 1 && number <= names.length) {
        ...
        onSelect((active + 1) % names.length);
      } else if (event.key === "ArrowLeft") {
        onSelect((active - 1 + names.length) % names.length);
  ...
  }, [active, names.length, onReplay, onSelect]);
...
export function updatePrototypeUrl(index: number) {
...
.proto-picker-item[data-active] {
  color: #fff; /* ds-allow — fixed prototype-picker harness spec */
}
