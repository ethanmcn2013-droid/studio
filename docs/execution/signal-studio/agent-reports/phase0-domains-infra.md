...
- **studio**: no Clerk on public surfaces; HQ via SIGNAL_HQ_PASSWORD cookie; no Stripe SDK; writes shared entitlements DB (STUDIO_*) via admin APIs; content = filesystem markdown; next.config.ts redirects: /students.html→/students, /brand→/design (permanent), /review→/hq/experience-quality (temp); no vercel.json.
