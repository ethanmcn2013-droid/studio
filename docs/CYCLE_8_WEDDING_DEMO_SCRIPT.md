...
**Format**: 60-second hero video for the `/weddings` page, venue outreach, and the analytics-demo Remotion repo. Two render targets: 16:9 (embed) + 9:16 (social).
...
**Music**: low-key cinematic piano + light strings, ~70 BPM, no drum kit. Reference: the Analytics typography demo's silence-as-signal register. If music is added, it should sit at -18 dB under the visuals and fade out by Scene 10. Avoid Epidemic Sound's "corporate inspiration" packs — too generic.
...
- Does not show the Analytics live briefing product (which doesn't exist yet — the demo points at the marketing artefact)
...
- Keep 9:16 in analytics-demo `out/` for social — Ethan posts manually
