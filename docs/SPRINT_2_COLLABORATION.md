...
| Signal Roadmap | One-way shareable artefacts — public link, no comments | Locked refusal in roadmap/docs/PRODUCT.md |
| Signal Notes | Single-user. Locked. | notes/docs/PRODUCT.md §7: "Not collaborative." |
| Signal Analytics | Private — the briefing is for one person | Analytics PRODUCT.md positioning |

**Implication:** Sprint 2 changes Tasks + Roadmap. It does NOT change Notes or Analytics.
...
4. **Plain-English activity.** Not "user_2k3 created entity 'task_abc' on workspace 'xyz'" — but "Ethan added two new venue notes 2 hours ago." Same discipline as Analytics's prose library.
...
| **10.2** | Guest view on Roadmap shareable artefacts — public link shows workspace name + creator + last-updated time | `roadmap/src/app/[workspaceSlug]/page.tsx` |
| **10.3** | "You're invited by X" context bar + one-tap reply on Roadmap shared views | `roadmap/src/components/roadmap/invited-by-bar.tsx` + a small Resend webhook |
| **10.4** | Plain-English activity log in Tasks (last 10 events, human prose) | new tasks table + `formatActivity` mapper |
| **10.5** | Cross-product invite: one invite → guest sees Tasks workspace + Roadmap shared update + (if exists) Analytics weekly | studio orchestration |
| **10.6** | Branded workspaces — venue/planner logo + accent on shared Roadmap view | roadmap workspace settings |
...
- **No notifications by default.** Opt-in only. Tasks/Roadmap stay silent by default.
- **No per-task permission matrix.** Workspace-level only. Granular permissions = configuration tax (BRAND.md §2.2).
- **No public directory of workspaces.** Roadmap PRODUCT.md locks this.
...
- **Analytics stays private.** The daily briefing is for one user. No "share my briefing" gesture in v1.
...
When a venue coordinator forwards the shared Roadmap link to a couple — and the couple opens it, understands what's happening, and replies — and the venue forwards three more couples that week — Sprint 2 has paid out.
...
4. **Email transport**: Resend (matches Roadmap) or Clerk-native invites? Default: Resend so the email is in the workspace creator's voice, not Clerk's.
